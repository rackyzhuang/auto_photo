...
AutoPhoto 是一款面向摄影师、修图师和影像团队的本地照片编辑软件，提供调色、AI 辅助调色、人像美化、男女妆容、预设风格、旋转裁切、分屏对比和 JPG 导出流程。
...
- 提供本地人像美化：瘦脸、瘦身、大眼、面部去皱、统一肤色、磨皮、肤色润色、美齿和衣物去褶皱。
- 提供一键统一肤色和一键美颜；使用人脸网格与多类别分割限制处理区域，保护五官、头发、衣物边界和原始明暗层次。
- 提供妆容模块：男士 10 款、女士 10 款，可调整统一强度、清除妆容，并随项目保存。妆容会分别处理底妆、唇色、腮红、眼影、眉色、眼线、轮廓与高光。
...
- Android / iPhone 移动端：提供触屏排版，只处理 JPG/JPEG；支持导入、预设、手动滑杆和数值输入、HSL、AI 调色、人像美化、男女妆容、旋转裁切、降噪、画质增强、撤销、缩放、眼睛对比和 JPG 导出。
...
5. 点击眼睛按钮查看前后对比。
6. 处理人像时，可使用一键美颜、手动美化参数，或在妆容模块选择男士/女士妆容并调整强度。
7. 根据需要旋转、裁切、复制参数，或使用参考图风格工具。
8. 如果需要 AI 调色或 AI 追色，在 AI 设置中填写自己的配置。
9. 设置导出参数，导出当前照片或批量导出选中的照片。

## AI 配置

AI 功能是可选能力。用户在软件内填写 API key 和 Base URL 后点击“确认设置”，软件会保存配置并检测连接；检测成功后配置区只保留模型选择和清空配置。

App 启动时只读取本地 AI 配置，不会自动联网检测。保存过配置后，只有用户打开 AI 面板时才会检测可用性。AI 调色和 AI 追色也只会在用户主动点击后运行。即使不配置 AI，AutoPhoto 仍然可以完成本地调色、人像美化、妆容、预设、裁切、旋转、对比和导出流程。

## 本地人像与妆容

人像分析使用随应用打包的 MediaPipe Tasks Vision 模型。脸部关键点用于瘦脸、大眼和妆容区域定位；多类别分割用于区分脸部皮肤、身体皮肤、头发和衣物。统一肤色只校正色度并保留亮度结构；磨皮采用保边平滑与细节回填；衣物去褶皱只减轻衣物区域的褶皱阴影，不会对整张照片做全局模糊。

人像分析和妆容渲染都在本机完成，不会上传照片。模型来源与许可证记录在 [`public/mediapipe/NOTICE.txt`](public/mediapipe/NOTICE.txt)。

## 移动端使用

移动端界面面向 Android 和 iPhone 的触屏操作，当前只接受 JPG/JPEG 图片。导入照片后，可以在底部工具栏切换 AI、预设、调色、美化、HSL、裁切和增强；美化面板包含男女妆容选择、妆容强度、一键美颜和完整人像参数。右下角提供缩放和眼睛对比按钮，顶部提供返回上一步和导出入口。
