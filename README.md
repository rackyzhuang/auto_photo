...
- macOS 桌面版：支持 Apple Silicon 和 Intel Mac，功能与桌面工作台一致。
...
GitHub Release 会生成 Windows、macOS、Android 和 iOS 构建产物。Android 会提供 APK/AAB，并额外提供可用于直接安装验证的 debug APK；iOS 默认提供模拟器应用包，真机 IPA 需要配置 Apple 签名证书和描述文件后生成。
